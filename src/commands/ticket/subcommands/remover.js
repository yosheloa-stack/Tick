...
export default {
  name: 'remover',
  description: 'Remove um membro do ticket atual.',
  /**
  ...
  register(subcommand) {
    return subcommand.addUserOption((option) =>
      option.setName('membro').setDescription('Membro que sera removido.').setRequired(true),
    );
  },
  /**
  ...
  async execute(interaction, context) {
    ...
    await context.services.tickets.removeParticipant({
      channel: interaction.channel,
      ticket,
      user,
    });

    await replySuccess(interaction, `${userMention(user.id)} nao tem mais acesso a este ticket.`);
  },
};
